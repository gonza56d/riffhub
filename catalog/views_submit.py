...
def submit_entry(request, kind):
    ...
    return render(
        ...
        "catalog/submit/form.html",
        {
            ...
            "label": label,
            "is_guitar": is_guitar,
            "pickups": Pickup.objects.all() if is_guitar else None,
...
def _attach_pickups(guitar, post) -> None:
    ...
    Silently ignores blank slots and any slot pointing at a missing pickup id;
    the form itself is the source of truth for the guitar's own fields, while
    pickups are optional extras chosen from existing catalog entries.
    """
    for field_name, position in PICKUP_SLOTS:
        pickup_id = post.get(field_name)
        if not pickup_id:
            continue
        pickup = Pickup.objects.filter(pk=pickup_id).first()
        if pickup is None:
            continue
